...
# ── Шаг 4: выпуск или обновление сертификата ──
...
# ── Шаг 5: оптимизация nginx.conf ──
echo -e "${GREEN}[4/6] Настройка nginx.conf (воркеры, соединения)...${NC}"

CPU_CORES=$(nproc)
# Читаем системный лимит fd; если не определяется — ставим 65535
MAX_FD=$(ulimit -Hn 2>/dev/null || echo 65535)
[[ -z "$MAX_FD" || "$MAX_FD" == "unlimited" ]] && MAX_FD=65535

# worker_connections = MAX_FD / 2, но не больше 65535 и не меньше 1024
...
http {
    ...
    # Производительность
    sendfile            on;
    tcp_nopush          on;    # заголовки + начало файла одним пакетом
    tcp_nodelay         on;    # отключить алгоритм Нейгла для keep-alive
...
# ── Шаг 6: конфиг виртуального хоста с HTTP/3 ──
