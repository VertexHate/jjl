...
# ════════════════════════════════════════════════
# [0/9] Удаление таймера авторестарта nginx
# ════════════════════════════════════════════════
echo -e "${GREEN}[0/9] Удаление таймера авторестарта nginx (если есть)...${NC}"
...
# ════════════════════════════════════════════════
# [1/9] Установка Nginx mainline с HTTP/3
# ════════════════════════════════════════════════
echo -e "${GREEN}[1/9] Установка Nginx mainline с поддержкой HTTP/3...${NC}"

apt update -y
apt install -y curl gnupg2 ca-certificates lsb-release python3
...
# ════════════════════════════════════════════════
# [2/9] Установка certbot
# ════════════════════════════════════════════════
echo -e "${GREEN}[2/9] Установка certbot...${NC}"
apt install -y certbot

# ════════════════════════════════════════════════
# [3/9] Выпуск или обновление сертификата
# ════════════════════════════════════════════════
echo -e "${GREEN}[3/9] Проверка SSL-сертификата для $ORIGIN_DOMAIN...${NC}"
...
# ════════════════════════════════════════════════
# [4/9] systemd override — лимиты для nginx
# ════════════════════════════════════════════════
echo -e "${GREEN}[4/9] Настройка systemd override для nginx (LimitNOFILE)...${NC}"
...
systemctl daemon-reload
echo -e "${GREEN}  ✔ systemd override применён (LimitNOFILE=65535, LimitNPROC=65535).${NC}"

# ════════════════════════════════════════════════
# [5/9] Docker daemon.json — лимиты для всех контейнеров (в т.ч. remnanode/Xray)
# ════════════════════════════════════════════════
echo -e "${GREEN}[5/9] Настройка лимитов Docker-демона (daemon.json)...${NC}"

DAEMON_JSON="/etc/docker/daemon.json"
mkdir -p /etc/docker

if [[ -f "$DAEMON_JSON" ]]; then
    echo -e "${CYAN}  Найден существующий daemon.json — мержим...${NC}"
    python3 - << 'PYEOF'
import json
...
# Перезапускаем Docker чтобы применить новые лимиты
echo -e "${CYAN}  Перезапуск Docker-демона...${NC}"
systemctl daemon-reload
systemctl restart docker
sleep 3

# Поднимаем remnanode обратно после рестарта docker
if docker inspect remnanode &>/dev/null; then
    ...
    echo -e "${GREEN}  ✔ Docker-демон перезапущен.${NC}"
    echo -e "${YELLOW}  ⚠ Контейнер remnanode не найден. Лимиты применятся при следующем запуске.${NC}"
fi

echo -e "${CYAN}  ℹ Если используешь docker-compose, добавь в сервис remnanode:${NC}"
echo -e "      ulimits:"
echo -e "        nofile:"
echo -e "          soft: 1048576"
echo -e "          hard: 1048576"
...
# ════════════════════════════════════════════════
# [6/9] limits.conf — системные лимиты
# ════════════════════════════════════════════════
echo -e "${GREEN}[6/9] Настройка системных лимитов (limits.conf)...${NC}"
...
echo -e "${GREEN}  ✔ /etc/security/limits.conf обновлён (nofile=1048576).${NC}"

# ════════════════════════════════════════════════
# [7/9] sysctl — параметры ядра
# ════════════════════════════════════════════════
echo -e "${GREEN}[7/9] Применение sysctl параметров...${NC}"
...
# ════════════════════════════════════════════════
# [8/9] nginx.conf — воркеры и производительность
# ════════════════════════════════════════════════
echo -e "${GREEN}[8/9] Настройка nginx.conf (воркеры, соединения)...${NC}"
...
# ════════════════════════════════════════════════
# [9/9] Очистка старых конфигов + новый конфиг + фейк-страница + HTTP/3 + фаервол
# ════════════════════════════════════════════════
echo -e "${GREEN}[9/9] Конфиг сайта, фейк-страница, HTTP/3, фаервол...${NC}"
...
<body>
    <div class="container">
        <h1>Welcome</h1>
        <p class="tagline">Your connection is secure</p>
        <div class="status-row">
            <span class="dot"></span>
            All systems operational
        </div>
        <div class="footer">nginx &middot; TLS 1.3</div>
    </div>
</body>
</html>
FAKEHTML

chown -R nginx:nginx /var/www/fake 2>/dev/null \
    || chown -R www-data:www-data /var/www/fake 2>/dev/null \
    || true
...
# ── Перезапуск ноды и финальная проверка ulimit ──
echo -e "${GREEN}[+] Перезапуск remnanode...${NC}"
if docker inspect remnanode &>/dev/null; then
    docker restart remnanode
    sleep 3
    ULIMIT_FINAL=$(docker exec remnanode sh -c "ulimit -n" 2>/dev/null || echo "н/д")
    echo -e "${GREEN}  ✔ remnanode перезапущен.${NC}"
    echo -e "${CYAN}  ulimit -n внутри remnanode: ${GREEN}${ULIMIT_FINAL}${NC}"
else
    echo -e "${YELLOW}  ⚠ Контейнер remnanode не найден, пропускаем.${NC}"
    ULIMIT_FINAL="н/д (контейнер не найден)"
fi
...
echo -e "  LimitNOFILE (systemd nginx): ${GREEN}65535${NC}"
echo -e "  nofile (limits.conf):        ${GREEN}1048576${NC}"
echo -e "  fs.file-max:                 ${GREEN}1000000${NC}"
echo -e "  somaxconn:                   ${GREEN}65535${NC}"
echo -e "  tcp_max_syn_backlog:         ${GREEN}65535${NC}"
echo -e "  netdev_max_backlog:          ${GREEN}65535${NC}"
echo ""
echo -e "${CYAN}Docker / remnanode (Xray):${NC}"
echo -e "  daemon.json default-ulimits: ${GREEN}1048576${NC}"
echo -e "  ulimit -n в контейнере:      ${GREEN}${ULIMIT_FINAL}${NC}"
echo ""
...
echo ""
echo -e "${CYAN}Страницы:${NC}"
echo -e "  Фейковая: ${GREEN}https://$ORIGIN_DOMAIN/${NC}"
echo -e "  Прокси:   ${GREEN}https://$ORIGIN_DOMAIN/api/user${NC}"
echo ""
